date: May 10 23:23:00 -0500 2009
categories: programming
summary: A way to avoid namespace collisions.

h1. Aikidoka Prevents Namespace Collisions

Recently I fell victim to the Twitter-Mash / Extlib-DataMapper-Mash "namespace collision":http://blog.zerosum.org/2009/4/17/why-namespaces-are-important. To get around this problem, I've created a new gem, "Aikidoka":http://github.com/flyingmachine/aikidoka/tree/master".

Here's what happened when I tried to use Twitter when Extlib had already been loaded:

bc(emphasize).. irb(main):001:0> require 'Twitter'
=> ["Twitter"]
irb(main):002:0> Twitter::Search.new("bokken").fetch
SystemStackError: stack level too deep

p. Here's what happens when you use Aikidoka:

bc(emphasize).. irb(main):001:0> require 'aikidoka'
=> ["Aikidoka"]
...
p. It works! What this does is namespace the Mash defined when I require the Twitter gem, so that Mash is now Twitter::Mash. Also, Extlib's Mash is still there, untouched, so you don't need to worry about that. Here's how Aikidoka does its magic:
